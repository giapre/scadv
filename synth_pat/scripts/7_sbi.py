...
type_of_extraction = 'traditional'
...
list3 = [pid for pid in os.listdir(f'{Paths.RESULTS}') if pid not in list1+list2+['sub-1280']]
for pid in ['sub-1280']: #os.listdir(f'{Paths.RESULTS}'):
    print(f'Running {pid}')

    if os.path.isfile(f'{Paths.RESULTS}/{pid}/{type_of_sweep}_{type_of_extraction}_estimated_mean_params.npz'):
        ...
        plot_sbi_violin_estimated_params(params_label, (njdopa_ctx_est, njdopa_str_est, ws_est), pid, type_of_sweep, type_of_extraction)
        plot_sbi_kde_distr(params_label, prior, posterior_samples, pid, type_of_sweep, type_of_extraction)